...
void checkSensorsState(){
    ...
    if (sensores_s[(char) i] == on && sensores_v[i] < tempInicial + tempMinVariacion) { 
      // lo apago
      sensores_s[i] = off;      
    }
    // el caso opuesto y la temperatura supero el umbral
    if (sensores_s[(char) i] == 0 && sensores_v[i] > tempInicial + tempMinVariacion ) { 
...
float getMVolts(uint16_t value) {
  return  5000.0  * (value * 1.00000 / 1024.0); //mv
}
